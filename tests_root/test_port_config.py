...
def test_dockerfile_port_expansion():
    ...
    assert (
        'ENTRYPOINT ["/app/docker-entrypoint.sh"]' in dockerfile_content
...
def test_port_environment_expansion():
    ...
    result = subprocess.run(
        ["sh", "-c", 'echo "PORT=${PORT:-8000}"'],
        capture_output=True,
        text=True,
        env={},
    )
    assert (
        result.stdout.strip() == "PORT=8000"
