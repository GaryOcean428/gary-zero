...
def test_routes(base_url):
    ...
    for test in test_cases:
        ...
        try:
            ...
            print(
                f"{status_icon} {test['method']:<7} {url:<30} - Status: {response.status_code}"
            )
    ...
    try:
        ...
        if failures:
            ...
            for failure in failures:
                if "error" in failure:
                    print(
                        f"  - {failure['method']} {failure['url']}: {failure['error']}"
                    )
                else:
                    print(
                        f"  - {failure['method']} {failure['url']}: Status {failure['status']} (expected {failure['expected']})"
                    )
