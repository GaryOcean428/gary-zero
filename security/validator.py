...
class SecureCodeValidator:
    ...
    def validate_code(self, request: CodeValidationRequest) -> ValidationResult:
        ...
        """
        result = ValidationResult(is_valid=True, security_level=request.security_level)
    ...
    def _check_imports(
        self, tree: ast.AST, result: ValidationResult, request: CodeValidationRequest
    ):
        """Check for unauthorized imports."""
        allowed = (
            request.allowed_modules or self.allowed_modules[request.security_level]
        )
            ...
            if isinstance(node, ast.Call):
                ...
                # Check method calls on dangerous modules
                    ...
                    if isinstance(node.func.value, ast.Name):
                        ...
                        if node.func.value.id in self.blocked_modules and hasattr(
                            node.func, "attr"
                        ):
                            result.blocked_items.append(
    ...
    def _check_dangerous_operations(self, tree: ast.AST, result: ValidationResult):
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Constant) and isinstance(node.value, str):
                if any(
                    dangerous in node.value.lower()
                    for dangerous in ["exec(", "eval(", "__import__"]
                ):
                    result.warnings.append(
                        "Potentially dangerous string content detected"
                    )
...
def validate_code(
    ...
    """
    request = CodeValidationRequest(code=code, security_level=security_level)
