...
def test_flask_routes(base_url):
    ...
    print(f"🧪 Comprehensive Flask Route Testing")
    ...
    critical_tests = [
        ...
        {'path': '/health', 'method': 'GET', 'critical': True, 'desc': 'Health check endpoint'},
        {'path': '/health', 'method': 'OPTIONS', 'critical': True, 'desc': 'Health CORS preflight'},
        {'path': '/ready', 'method': 'GET', 'critical': True, 'desc': 'Readiness check endpoint'},
        {'path': '/ready', 'method': 'OPTIONS', 'critical': True, 'desc': 'Ready CORS preflight'},
        
        # Root endpoint (main application)
        {'path': '/', 'method': 'GET', 'critical': True, 'desc': 'Main page GET'},
        {'path': '/', 'method': 'POST', 'critical': True, 'desc': 'Form submission to root'},
        {'path': '/', 'method': 'OPTIONS', 'critical': True, 'desc': 'Root CORS preflight'},
        
        # API endpoint
        {'path': '/api', 'method': 'GET', 'critical': False, 'desc': 'API endpoint GET'},
        {'path': '/api', 'method': 'POST', 'critical': False, 'desc': 'API endpoint POST'},
        {'path': '/api', 'method': 'OPTIONS', 'critical': False, 'desc': 'API CORS preflight'},
        
        # Static content
        {'path': '/privacy', 'method': 'GET', 'critical': False, 'desc': 'Privacy policy'},
        {'path': '/privacy', 'method': 'OPTIONS', 'critical': False, 'desc': 'Privacy CORS preflight'},
        {'path': '/terms', 'method': 'GET', 'critical': False, 'desc': 'Terms of service'},
        {'path': '/terms', 'method': 'OPTIONS', 'critical': False, 'desc': 'Terms CORS preflight'},
        {'path': '/favicon.ico', 'method': 'GET', 'critical': False, 'desc': 'Favicon'},
        {'path': '/favicon.ico', 'method': 'OPTIONS', 'critical': False, 'desc': 'Favicon CORS preflight'},
        
        # Debug endpoint
        {'path': '/debug/routes', 'method': 'GET', 'critical': False, 'desc': 'Route debugging'},
        {'path': '/debug/routes', 'method': 'OPTIONS', 'critical': False, 'desc': 'Debug CORS preflight'},
        
        # Error handling tests
        {'path': '/nonexistent', 'method': 'GET', 'critical': False, 'desc': '404 error handling'},
    ]
    ...
    for test in critical_tests:
        ...
        if result.get('status') == 'CONN':
            ...
            status_display = str(result['status'])
            icon = "⚠️"
            success = result['status'] in [404] and not test['critical']  # 404 is ok for nonexistent routes
...
def test_with_auth(base_url):
    ...
    for test in auth_tests:
        ...
        if result.get('status') == 401:
            ...
            print(f"📖 {test['desc']}: Accessible without auth (status: {result.get('status')})")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  python test_flask_routes.py https://gary-zero-production.up.railway.app")
        sys.exit(1)
