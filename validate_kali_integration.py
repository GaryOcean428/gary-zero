...
def test_shell_command_execution() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'success': False,
            'error': f'Command execution test failed: {str(e)}'
        }
...
def validate_framework_integration() -> Dict[str, Any]:
    ...
    results['integration_complete'] = (
        results['all_files_present'] and 
        results.get('prompt_integration', {}).get('shell_integration_included', False)
    )
...
async def test_kali_executor_integration() -> Dict[str, Any]:
    ...
    try:
        ...
        if not results['availability_check']:
            results['executor_tests']['note'] = 'Kali service not available - skipping executor tests'
            return results
        ...
        if init_success:
            # Test basic command execution
            try:
                cmd_result = await executor.execute_command('echo "Hello from Kali"', timeout=10)
...
def generate_integration_report(results: Dict[str, Any]) -> str:
        ...
        if executor_results.get('availability_check'):
            ...
            if executor_tests.get('initialization', {}).get('success'):
                ...
                if executor_tests.get('tools_check', {}).get('success'):
                    tools_count = executor_tests.get('tools_check', {}).get('tools_found', 0)
                    report_lines.append(f"  ✅ Security tools available ({tools_count} found)")
            else:
    ...
    if passed_checks < total_checks:
        ...
        if not env_results.get('all_required_present'):
            report_lines.append("  • Configure missing environment variables in Railway service")
        
        if not conn_results.get('connectivity'):
            report_lines.append("  • Check Kali service deployment and network configuration")
            
        if not cmd_results.get('success'):
            report_lines.append("  • Verify Kali service API endpoints and authentication")
            
        if not framework_results.get('integration_complete'):
            report_lines.append("  • Ensure all framework integration files are properly deployed")
            
        if not executor_results.get('import_successful'):
            report_lines.append("  • Check Python environment and framework dependencies")
...
async def main():
    ...
    env_status = "✅ All required variables present" if results['environment']['all_required_present'] else "❌ Missing required variables"
    print(f"   {env_status}")
    ...
    conn_status = "✅ Service reachable" if results['connectivity']['connectivity'] else "❌ Connection failed"
    print(f"   {conn_status}")
    ...
    if results['connectivity']['connectivity']:
        ...
        cmd_status = "✅ Commands working" if results['command_execution']['success'] else "❌ Command execution failed"
        print(f"   {cmd_status}")
    else:
        print("\n💻 Skipping command execution test (no connectivity)")
        results['command_execution'] = {'success': False, 'error': 'No connectivity to test'}
    
    # Test 4: Framework Integration
    print("\n🔧 Validating framework integration...")
    results['framework_integration'] = validate_framework_integration()
    
    framework_status = "✅ Integration complete" if results['framework_integration']['integration_complete'] else "❌ Integration incomplete"
    ...
    executor_status = "✅ Executor working" if results['executor_integration']['import_successful'] else "❌ Executor issues"
    ...
    overall_success = (
        results['environment']['all_required_present'] and
        results['connectivity']['connectivity'] and
        results['command_execution']['success'] and
        results['framework_integration']['integration_complete']
    )
