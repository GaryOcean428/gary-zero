...
class GeminiLiveTool:
    ...
    async def _handle_status(self) -> Response:
        ...
        model = self.args.get("model", "models/gemini-2.5-flash-preview-native-audio-dialog")
    ...
    async def _handle_configure(self) -> Response:
        ...
        if config_changes:
            message = f"**Configuration Updated**\n" + "\n".join(f"• {change}" for change in config_changes)
        else:
            ...
            message += f"• Default Model: models/gemini-2.5-flash-preview-native-audio-dialog"
    ...
    async def _handle_start_streaming(self) -> Response:
        ...
        if self.is_streaming:
            return Response(
                message="⚠️ Streaming session already active",
                break_loop=False
            )
    ...
    async def _handle_stop_streaming(self) -> Response:
        ...
        if not self.is_streaming:
            return Response(
                message="⚠️ No active streaming session to stop",
                break_loop=False
            )
    ...
    async def _handle_send_audio(self) -> Response:
        ...
        return Response(
            message="❌ No client available to send audio",
            break_loop=False
        )
