...
class TestAuthRateLimiter(unittest.TestCase):
    ...
    def test_auth_rate_limiting(self):
        ...
        for i in range(3):
            self.assertTrue(self.limiter.is_auth_allowed(ip), f"Auth attempt {i+1} should be allowed")
        
        # 4th attempt should be blocked
        self.assertFalse(self.limiter.is_auth_allowed(ip), "4th auth attempt should be blocked")
    ...
    def test_success_log_rate_limiting(self):
        ...
        for i in range(2):
            self.assertTrue(self.limiter.should_log_success(ip), f"Success log {i+1} should be allowed")
        
        # 3rd log should be blocked
        self.assertFalse(self.limiter.should_log_success(ip), "3rd success log should be blocked")
