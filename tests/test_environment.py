    ...
    def create_mock_api_service(self, service_name: str) -> Mock:
        ...
        if service_name == "openai":
            ...
            mock_instance.run_code = MagicMock(
                return_value=Mock(
                    stdout="Mock code execution output",
                    stderr="",
                    exit_code=0
                )
            )
...
def mock_performance_metrics():
    ...
    mock_metrics.get_latest = MagicMock(return_value=Mock(value=0.1, timestamp=1234567890))
...
def generate_performance_test_data(metric_count: int = 1000) -> list:
    ...
    for i in range(metric_count):
        metrics.append({
            ...
            "tags": {"category": f"cat_{i % 5}", "priority": ["low", "medium", "high"][i % 3]},
            "unit": "seconds"
        })
