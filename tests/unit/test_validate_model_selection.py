...
class TestValidateModelSelection:
    ...
    def test_embedding_model_without_modern_flag_passes(self, mock_get_models):
        ...
        assert (
            validate_model_selection("TEST_PROVIDER", "text-embedding-custom") is True
        )
    ...
    def test_case_insensitive_embedding_detection(self):
        ...
        with patch(
            "framework.helpers.model_catalog.get_models_for_provider"
        ) as mock_get_models:
            ...
            assert (
                validate_model_selection("TEST_PROVIDER", "TEXT-EMBEDDING-UPPER")
                is True
            )
            assert (
                validate_model_selection("TEST_PROVIDER", "text-Embedding-mixed")
                is True
            )
    ...
    def test_embedding_in_middle_of_name(self):
        ...
        with patch(
            ...
            assert (
                validate_model_selection("TEST_PROVIDER", "custom-embedding-v2") is True
            )
    ...
    def test_both_modern_and_embedding_passes(self, mock_get_models):
        ...
        assert (
            validate_model_selection("TEST_PROVIDER", "modern-text-embedding") is True
        )
    ...
    def test_unreleased_gpt41_mini_model_removed(self):
        """Test that unreleased gpt-4.1-mini model is not in catalog."""
        from framework.helpers.model_catalog import is_valid_model_for_provider

        # GPT-4.1 Mini should return False as it was removed for being unreleased
        assert is_valid_model_for_provider("OPENAI", "gpt-4.1-mini") is False

        # Test that other GPT-4.1 series models are also removed
        assert is_valid_model_for_provider("OPENAI", "gpt-4.1") is False
        assert is_valid_model_for_provider("OPENAI", "gpt-4.1-nano") is False
        assert is_valid_model_for_provider("OPENAI", "gpt-4.1-vision") is False
        assert is_valid_model_for_provider("OPENAI", "chatgpt-4.1") is False

        # Test that valid models still work
        assert is_valid_model_for_provider("OPENAI", "gpt-4o-mini") is True
        assert is_valid_model_for_provider("OPENAI", "o1-mini") is True
