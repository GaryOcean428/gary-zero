...
class TestPerformanceMetric:
    ...
    def test_metric_defaults(self):
        ...
        timestamp = time.time()
        metric = PerformanceMetric(name="test_metric", value=5.0, timestamp=timestamp)
...
class TestResourceTracker:
    ...
    def test_get_resource_snapshot_error_handling(self):
        ...
        with patch(
            "framework.performance.monitor.psutil.cpu_percent",
...
class TestPerformanceMonitor:
    ...
    async def test_async_timed_decorator(self, monitor):
        ...
        # Check that timing was recorded
        latest = monitor.metrics.get_latest(
            "operation_duration_async_decorated_function"
        )
        assert latest is not None
        assert latest.value >= 0.05
...
class TestGlobalFunctions:
    ...
    def test_global_timer_decorator(self):
        ...
        latest = global_monitor.metrics.get_latest(
            "operation_duration_global_test_operation"
        )
        assert latest is not None
    ...
    async def test_global_async_timer_decorator(self):
        ...
        global_monitor = get_performance_monitor()
        latest = global_monitor.metrics.get_latest(
            "operation_duration_global_async_test_operation"
        )
        assert latest is not None
        assert latest.value >= 0.01
