...
class TestDirectoryHealthLogic(unittest.TestCase):
    ...
    def test_required_files_validation(self):
        ...
        required_files = {
            "settings/config.json": os.path.join(self.test_dir, "settings", "config.json"),
            "memory/context.json": os.path.join(self.test_dir, "memory", "context.json"),
            "scheduler/tasks.json": os.path.join(self.test_dir, "scheduler", "tasks.json"),
        }
