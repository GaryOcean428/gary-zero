...
def test_railway_routes(base_url):
    ...
    critical_tests = [
        ...
        {'path': '/', 'method': 'GET', 'critical': True, 'desc': 'Main page GET request'},
        ...
        {'path': '/health', 'method': 'OPTIONS', 'critical': False, 'desc': 'CORS preflight for health'},
        {'path': '/api', 'method': 'OPTIONS', 'critical': False, 'desc': 'CORS preflight for API'},
        {'path': '/', 'method': 'OPTIONS', 'critical': False, 'desc': 'CORS preflight for root'},
    ...
    for test in critical_tests:
        try:
            ...
            if test['method'] == 'GET':
                response = requests.get(f"{base_url}{test['path']}", 
            ...
            if response.status_code == 405:
                ...
                print(f"{'❌ ' + test['desc']:<40} {test['method']:<8} {response.status_code:<8} METHOD NOT ALLOWED")
                ...
                # Success
                print(f"{'✅ ' + test['desc']:<40} {test['method']:<8} {response.status_code:<8} OK")
                results['passed'] += 1
                
            elif response.status_code == 404 and test['path'] == '/nonexistent':
                # Expected 404
                print(f"{'✅ ' + test['desc']:<40} {test['method']:<8} {response.status_code:<8} OK (expected)")
                results['passed'] += 1
                
            elif response.status_code in [401, 403]:
                # Authentication required - not a routing issue
                print(f"{'🔐 ' + test['desc']:<40} {test['method']:<8} {response.status_code:<8} AUTH REQUIRED")
                results['passed'] += 1
                
            elif response.status_code >= 500:
                # Server error
                print(f"{'⚠️ ' + test['desc']:<40} {test['method']:<8} {response.status_code:<8} SERVER ERROR")
                results['failed'] += 1
                if test['critical']:
                    results['critical_failed'] += 1
                ...
                # Other client error
                print(f"{'⚠️ ' + test['desc']:<40} {test['method']:<8} {response.status_code:<8} CLIENT ERROR")
                results['failed'] += 1
                
        except requests.exceptions.ConnectionError:
            print(f"{'❌ ' + test['desc']:<40} {test['method']:<8} {'CONN':<8} CONNECTION FAILED")
            results['failed'] += 1
            if test['critical']:
                results['critical_failed'] += 1
                
        except requests.exceptions.Timeout:
            print(f"{'❌ ' + test['desc']:<40} {test['method']:<8} {'TIMEOUT':<8} REQUEST TIMEOUT")
            results['failed'] += 1
            if test['critical']:
                results['critical_failed'] += 1
                
        except Exception as e:
            print(f"{'❌ ' + test['desc']:<40} {test['method']:<8} {'ERROR':<8} {str(e)[:20]}")
            results['failed'] += 1
            if test['critical']:
                results['critical_failed'] += 1
            ...
            if scenario['should_be_405'] and response.status_code == 405:
                ...
                try:
                    ...
                    if 'allowed_methods' in error_data and 'message' in error_data:
                        print(f"   ✅ Enhanced error details provided")
                    else:
                        print(f"   ⚠️ Basic 405 response (could be enhanced)")
                except:
                    print(f"   ⚠️ Non-JSON 405 response")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Example: python test_railway_routes.py https://gary-zero-production.up.railway.app")
