...
def parse_redis_host(redis_url: str) -> str:
    ...
    try:
        ...
        if parsed.hostname:
            ...
            fallback = redis_url.replace('redis://', '').replace('rediss://', '').split('/')[0].split('@')[-1]
            ...
            if (not fallback or 
                (fallback not in ['localhost'] and 
                 not fallback.endswith('.internal') and 
                 '.' not in fallback)):
                return 'redis-j-ux.railway.internal'
    ...
    for attempt in range(retry_count):
        try:
            ...
            if attempt < retry_count - 1:
                ...
                safe_url = mask_sensitive_url(redis_url) if redis_url else "host-based connection"
                logger.warning(f"Redis connection failed (attempt {attempt + 1}/{retry_count}) for {safe_url}. Retrying in {wait_time}s...")
                time.sleep(wait_time)
            else:
                safe_url = mask_sensitive_url(redis_url) if redis_url else "host-based connection"
                logger.error(f"Failed to connect to Redis after {retry_count} attempts for {safe_url}")
                return None
...
def get_postgres_connection():
    ...
    try:
        ...
        safe_url = mask_sensitive_url(database_url) if database_url else "host-based connection"
