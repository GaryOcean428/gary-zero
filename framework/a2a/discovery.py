...
class DiscoveryRequest(BaseModel):
    ...
    capabilities_filter: Optional[List[str]] = Field(description="Filter by capabilities", default=None)
    protocols_filter: Optional[List[str]] = Field(description="Filter by protocols", default=None)


class DiscoveryResponse(BaseModel):
    """A2A discovery response model"""
    success: bool = Field(description="Whether discovery was successful")
    agent_card: Optional[AgentCard] = Field(description="Agent card information", default=None)
    filtered_capabilities: Optional[List[str]] = Field(description="Filtered capabilities", default=None)
    available_services: Optional[List[str]] = Field(description="Available services", default=None)
    error: Optional[str] = Field(description="Error message if discovery failed", default=None)
...
class DiscoveryService:
    ...
    def discover(self, request: DiscoveryRequest) -> DiscoveryResponse:
        ...
        try:
            ...
            if request.capabilities_filter:
                available_caps = [cap.name for cap in agent_card.capabilities if cap.enabled]
                filtered_capabilities = [cap for cap in request.capabilities_filter if cap in available_caps]
            
            # Apply protocol filters if provided  
            available_protocols = agent_card.protocols
            if request.protocols_filter:
                available_protocols = [proto for proto in agent_card.protocols 
    ...
    def get_capability_details(self, capability_name: str) -> Optional[Dict[str, Any]]:
        """
        ...
        for capability in agent_card.capabilities:
            if capability.name == capability_name and capability.enabled:
                return {
                    ...
                    "endpoints": [
                        endpoint.dict() for endpoint in agent_card.endpoints
                        if capability_name in endpoint.description.lower() or 
                           capability_name in endpoint.name.lower()
                    ]
                }
    ...
    def get_protocol_details(self, protocol_name: str) -> Optional[Dict[str, Any]]:
        ...
        if protocol_name == "a2a":
            ...
            protocol_details["features"] = ["discovery", "negotiation", "messaging", "streaming"]
            ...
            protocol_details["features"] = ["method_calls", "notifications", "batch_requests"]
