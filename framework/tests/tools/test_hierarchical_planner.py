...
class TestPlanEvaluator:
    ...
    def test_evaluate_failed_output(self):
        ...
        evaluator = PlanEvaluator()
        subtask = Subtask(
            id="test_id", name="Test task", description="Test description"
        )
...
class TestPlanAdjuster:
    ...
    def test_retry_with_alternative_tool(self):
        ...
        result = adjuster.adjust_plan_after_failure(
            plan, failed_subtask, evaluation_result
        )
    ...
    def test_split_complex_subtask(self):
        ...
        failed_subtask = plan.subtasks[0]
        evaluation_result = EvaluationResult(
            success=False, score=0.2, feedback="Task too complex", requires_retry=True
        )
...
class TestEvaluationLoop:
    ...
    def test_process_successful_completion(self):
        ...
        plan = HierarchicalPlan(
            ...
            subtasks=[
                Subtask(
                    id="test_subtask", name="Test task", description="Test description"
                )
            ],
        )
...
class TestPlanningTool:
    ...
    def planning_tool(self):
        ...
        tool = PlanningTool(
            agent=MagicMock(), name="planning_tool", method="create_plan"
        )
    ...
    async def test_get_plan_status(self, planning_tool):
        ...
        # First create a plan
        planning_tool.method = "create_plan"
        create_response = await planning_tool.execute(objective="Test objective")
    ...
    async def test_configure_planner(self, planning_tool):
        ...
        response = await planning_tool.execute(
            auto_planning_enabled=False, max_subtasks=5
        )
