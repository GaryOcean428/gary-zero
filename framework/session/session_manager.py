...
class RemoteSessionManager:
    ...
    async def create_session(
        self, session_type: SessionType, **kwargs
    ) -> SessionInterface:
        """
        ...
        if session_type not in self._session_factories:
            raise ValueError(
                f"No factory registered for session type: {session_type.value}"
            )

        factory = self._session_factories[session_type]
        session = await self.connection_pool.get_session(
            session_type, factory, **kwargs
        )

        self.logger.info(
            f"Created session {session.session_id} of type {session_type.value}"
        )
        return session
    ...
    def create_message(
        self, message_type: str, payload: dict[str, Any], session_id: str | None = None
    ) -> SessionMessage:
        """
    ...
    async def _request_approval(
        self, session: SessionInterface, message: SessionMessage
    ) -> bool:
        """
        ...
        self.logger.warning(
            f"No approval handler configured for {session.session_type.value}"
        )
        return False
    ...
    async def _store_output(
        ...
        if output_size > self.config.max_output_size:
            self.logger.warning(
                f"Output size {output_size} exceeds limit {self.config.max_output_size}"
            )
            return
    ...
    async def _cleanup_old_outputs(self):
        ...
        cutoff_time = datetime.now() - timedelta(
            seconds=self.config.output_retention_time
        )
        keys_to_remove = []
