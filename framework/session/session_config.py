...
class SessionConfig:
    ...
    def from_environment(cls) -> "SessionConfig":
        ...
        config.health_check_interval = int(
            os.getenv("SESSION_HEALTH_CHECK_INTERVAL", "60")
        )

        # Connection pooling
        config.enable_connection_pooling = (
            os.getenv("SESSION_ENABLE_POOLING", "true").lower() == "true"
        )
        config.pool_size_per_type = int(os.getenv("SESSION_POOL_SIZE", "5"))
        config.max_idle_time = int(os.getenv("SESSION_MAX_IDLE_TIME", "300"))

        # Security and approval
        config.require_approval_for_gui = (
            os.getenv("SESSION_REQUIRE_GUI_APPROVAL", "true").lower() == "true"
        )
        config.require_approval_for_terminal = (
            os.getenv("SESSION_REQUIRE_TERMINAL_APPROVAL", "true").lower() == "true"
        )
        config.require_approval_for_external = (
            os.getenv("SESSION_REQUIRE_EXTERNAL_APPROVAL", "true").lower() == "true"
        )
        config.approval_timeout = int(os.getenv("SESSION_APPROVAL_TIMEOUT", "30"))

        # Memory and storage
        config.store_outputs_in_memory = (
            os.getenv("SESSION_STORE_OUTPUTS", "true").lower() == "true"
        )
        config.max_output_size = int(
            os.getenv("SESSION_MAX_OUTPUT_SIZE", str(1024 * 1024))
        )
        config.output_retention_time = int(
            os.getenv("SESSION_OUTPUT_RETENTION", "3600")
        )
        ...
        config.gemini_cli_config = {
            ...
            "auto_install": os.getenv("GEMINI_CLI_AUTO_INSTALL", "false").lower()
            == "true",
            ...
            "response_modalities": os.getenv(
                "GEMINI_LIVE_RESPONSE_MODALITIES", "AUDIO"
            ),
        }

        config.anthropic_computer_use_config = {
            "enabled": os.getenv("ANTHROPIC_COMPUTER_USE_ENABLED", "false").lower()
            == "true",
            ...
            "screenshot_interval": float(
                os.getenv("ANTHROPIC_COMPUTER_USE_SCREENSHOT_INTERVAL", "1.0")
            ),
            "max_actions_per_session": int(
                os.getenv("ANTHROPIC_COMPUTER_USE_MAX_ACTIONS", "50")
            ),
        ...
        config.claude_code_config = {
            ...
            "max_file_size": int(
                os.getenv("CLAUDE_CODE_MAX_FILE_SIZE", str(1024 * 1024))
            ),
            "allowed_extensions": os.getenv(
                "CLAUDE_CODE_ALLOWED_EXTENSIONS",
                ".py,.js,.ts,.html,.css,.json,.md,.txt,.yml,.yaml,.toml",
            ).split(","),
            "restricted_paths": os.getenv(
                "CLAUDE_CODE_RESTRICTED_PATHS",
                ".git,node_modules,__pycache__,.venv,venv",
            ).split(","),
            "enable_git_ops": os.getenv("CLAUDE_CODE_ENABLE_GIT", "true").lower()
            == "true",
            "enable_terminal": os.getenv("CLAUDE_CODE_ENABLE_TERMINAL", "true").lower()
            == "true",
        }
