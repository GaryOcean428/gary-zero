...
class KaliSession(SessionInterface):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.username = config.get("username") or os.getenv(
            "KALI_USERNAME", "GaryOcean"
        )
        self.password = config.get("password") or os.getenv(
    ...
    async def connect(self) -> SessionResponse:
        ...
        try:
            ...
            response = self.http_session.get(
                urljoin(self.base_url, "/health"), timeout=10
            )
            if response.status_code == 200:
                ...
                try:
                    info_response = self.http_session.get(
                        urljoin(self.base_url, "/info"), timeout=5
                    )
                    service_info = (
                        info_response.json() if info_response.status_code == 200 else {}
                    )
                except Exception:
                    service_info = {}
                ...
                await self.update_state(
                    SessionState.ERROR, f"HTTP {response.status_code}"
                )
                return SessionResponse(
                    success=False,
    ...
    async def health_check(self) -> SessionResponse:
        ...
        try:
            ...
            response = self.http_session.get(
                urljoin(self.base_url, "/health"), timeout=5
            )
            if response.status_code == 200:
                return SessionResponse(
    ...
    async def _get_tools(self) -> SessionResponse:
        ...
        return await self._execute_command({"command": command, "timeout": 10})
    ...
    async def _run_audit(self, payload: dict[str, Any]) -> SessionResponse:
        ...
        results["port_scan"] = (
            port_scan.data if port_scan.success else {"error": port_scan.error}
        )
        ...
        if port_scan.success and port_scan.data:
            ...
            if any(port in stdout for port in ["80/tcp", "443/tcp", "8080/tcp"]):
                nikto_result = await self._execute_command(
                    {"command": f"nikto -h {target}", "timeout": 180}
                )
                results["web_scan"] = (
                    nikto_result.data
                    if nikto_result.success
                    else {"error": nikto_result.error}
                )
