...
async def initialize_cli_tools(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        PrintStyle(font_color="#85C1E9", bold=True).print("🔧 Initializing CLI tools...")
        ...
        PrintStyle(font_color="#FF6B6B").print(f"❌ CLI initialization failed: {str(e)}")
        return {"error": str(e)}
...
def setup_cli_paths_in_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        PrintStyle(font_color="#FF6B6B").print(f"❌ Error updating config with CLI paths: {str(e)}")
        return config
