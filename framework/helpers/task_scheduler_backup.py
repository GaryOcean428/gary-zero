    ...
    async def _run_task(
        ...
        async def _run_task_wrapper(task_uuid: str, task_context: str | None = None):
            ...
            if current_task.state != TaskState.RUNNING:
                ...
                self._printer.print(
                    f"Scheduler Task '{current_task.name}' state is "
                    f"'{current_task.state}', skipping"
                )
                return
            ...
            try:
                ...
                if updated_task and updated_task.state != TaskState.IDLE:
                    self._printer.print(
                        f"Fixing task state consistency: '{current_task.name}' "
                        f"state is not IDLE after success"
                    )
                    await self.update_task(task_uuid, state=TaskState.IDLE)
    ...
    if not task_class:
        ...
        if task_type_str == "scheduled":
            ...
            if not task_data.get("token"):
                task_data["token"] = str(uuid.uuid4().int)
