...
class PlannerSettings:
    ...
    def validate_config(cls, config_dict: Dict[str, Any]) -> Dict[str, str]:
        ...
        if "max_recursion_depth" in config_dict:
            if not isinstance(config_dict["max_recursion_depth"], int) or config_dict["max_recursion_depth"] < 1:
                errors["max_recursion_depth"] = "Must be a positive integer"
                
        if "max_subtasks" in config_dict:
            if not isinstance(config_dict["max_subtasks"], int) or config_dict["max_subtasks"] < 1:
                errors["max_subtasks"] = "Must be a positive integer"
                
        if "model_name" in config_dict:
            if not isinstance(config_dict["model_name"], str) or not config_dict["model_name"].strip():
