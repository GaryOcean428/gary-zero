...
class ModelRegistry:
    ...
    def _initialize_registry(self):
        ...
        for provider_key, models in MODEL_CATALOG.items():
            ...
            for model_info in models:
                ...
                if any(
                    keyword in model_value.lower()
                    for keyword in ["gpt-4", "claude-3", "claude-4", "o3", "gpt-5"]
                ):
                ...
                if any(
                    keyword in model_value.lower()
                    for keyword in ["gpt-5", "claude-4", "o3"]
                ):
                ...
                if (
                    any(
                        keyword in model_value.lower()
                        for keyword in ["gpt-5", "claude-4"]
                    )
                    or "claude" in model_value.lower()
                ):
                    max_tokens = 200000
    ...
    def validate_model(self, qualified_id: str) -> bool:
        ...
        if qualified_id not in self._registry:
            PrintStyle().error(
                f"Unknown model '{qualified_id}'. Available models: {list(self._registry.keys())}"
            )
            return False
        return True

    def get_model_config(self, qualified_id: str) -> ModelConfig | None:
    ...
    def get_utility_model(self, settings) -> ModelConfig | None:
        ...
        try:
            ...
            if util_provider and util_model_name:
                ...
                if self.validate_model(qualified_id):
                    ...
                    PrintStyle().warn(
                        f"⚠️ Configured utility model not in registry: {qualified_id}"
                    )
...
def get_utility_model_config(settings) -> ModelConfig | None:
