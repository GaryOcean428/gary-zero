...
class PromptFileHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        if not event.is_directory and event.src_path.endswith(('.txt', '.md', '.yaml', '.yml', '.json')):
...
class DynamicPromptLoader:
    ...
    def start_watching(self):
        ...
        try:
            ...
            self.print_style.success(f"Started watching prompts directory: {self.prompts_dir}")
    ...
    def _parse_prompt_file(self, file_path: str, content: str) -> Optional[Dict[str, Any]]:
    ...
    def get_prompt(self, prompt_path: str) -> Optional[Dict[str, Any]]:
        """Get a prompt by its relative path."""
        return self.prompts_cache.get(prompt_path)
    
    def get_agent_prompt(self, agent_id: str) -> Optional[Dict[str, Any]]:
