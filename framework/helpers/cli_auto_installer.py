...
class CLIInstaller:
    ...
    def __init__(self, name: str, config: Dict[str, Any]):
        self.name = name
        self.config = config
        self.tmp_bin_path = Path("/tmp/bin")
        self.tmp_bin_path.mkdir(exist_ok=True)

    async def detect_cli(self) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            if result["success"]:
                PrintStyle(font_color="#85C1E9").print(f"✅ {self.name} CLI found: {result['output']}")
            ...
            if not os.path.isabs(cli_path):
                ...
                if result["success"] and result["output"].strip():
                    ...
                    PrintStyle(font_color="#85C1E9").print(f"✅ {self.name} CLI found at: {actual_path}")
            ...
            if tmp_cli_path.exists():
                result = await self._run_command([str(tmp_cli_path), "--version"], timeout=10)
                if result["success"]:
                    PrintStyle(font_color="#85C1E9").print(f"✅ {self.name} CLI found in /tmp/bin: {result['output']}")
    ...
    async def auto_install(self) -> Tuple[bool, str]:
        ...
        try:
            PrintStyle(font_color="#FFA500").print(f"📦 Auto-installing {self.name} CLI to /tmp/bin...")
    ...
    async def _run_command(self, cmd: List[str], timeout: int = 60, cwd: Optional[str] = None) -> Dict[str, Any]:
        """
            ...
            if available:
                ...
                if success:
                    ...
                    if available:
                        ...
                        result["error"] = f"Installation succeeded but CLI still not detected: {path_or_error}"
                else:
