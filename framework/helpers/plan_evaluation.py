...
class EvaluationCriteria(BaseModel):
    ...
    expected_format: Optional[str] = None  # json, markdown, html, etc.
    custom_validator: Optional[str] = None  # custom validation logic
...
class PlanEvaluator:
    ...
    def evaluate_subtask_output(
        ...
        if criteria and criteria.min_length and output_length < criteria.min_length:
            score -= 0.2
            feedback_parts.append(f"Output too short ({output_length} < {criteria.min_length} chars)")
            ...
            score -= 0.1
            feedback_parts.append(f"Output too long ({output_length} > {criteria.max_length} chars)")
        ...
        if criteria and criteria.required_keywords:
            ...
            if missing_keywords:
                score -= 0.3
                feedback_parts.append(f"Missing required keywords: {', '.join(missing_keywords)}")
                suggestions.append(f"Include information about: {', '.join(missing_keywords)}")
            else:
                score += 0.2
        ...
        if criteria and criteria.forbidden_keywords:
            ...
            if found_forbidden:
                score -= 0.2
                feedback_parts.append(f"Contains forbidden keywords: {', '.join(found_forbidden)}")
        ...
        if criteria and criteria.expected_format:
            ...
            if format_valid:
                ...
                score -= 0.3
                feedback_parts.append(f"Output does not match expected format: {criteria.expected_format}")
        ...
        feedback = "; ".join(feedback_parts) if feedback_parts else "Output meets basic criteria"
    ...
    def _evaluate_by_tool(self, subtask: Subtask, output: str) -> 'ToolEvaluationResult':
    ...
    def _evaluate_knowledge_output(self, output: str) -> 'ToolEvaluationResult':
        ...
        if "?" in output and any(word in output.lower() for word in ["answer", "solution", "result"]):
            score_adjustment += 0.1
...
class PlanAdjuster:
    ...
    def adjust_plan_after_failure(
        ...
        """
        logger.info(f"Adjusting plan {plan.id} after subtask failure: {failed_subtask.name}")
    ...
    def _retry_with_alternative_tool(
        ...
        failed_index = next(
            i for i, subtask in enumerate(plan.subtasks) 
            ...
            if len(parts) > 1:
                ...
                for i, part in enumerate(parts):
                    new_subtask = Subtask(
                        ...
                        dependencies=failed_subtask.dependencies.copy() if i == 0 else [prev_id]
                    )
...
class EvaluationLoop:
    ...
    def process_subtask_completion(
        ...
        if evaluation.success:
            ...
            logger.info(f"Subtask '{subtask.name}' completed successfully (score: {evaluation.score:.2f})")
            return True
        else:
            logger.warning(f"Subtask '{subtask.name}' failed evaluation (score: {evaluation.score:.2f}): {evaluation.feedback}")
            
            if evaluation.requires_retry:
                # Try to adjust the plan
                adjusted = self.adjuster.adjust_plan_after_failure(plan, subtask, evaluation)
                if adjusted:
                    logger.info(f"Plan adjusted after subtask failure")
                    return False
                else:
                ...
                subtask.mark_completed(output)
                logger.info(f"Subtask completed with low score but accepted")
                return True
