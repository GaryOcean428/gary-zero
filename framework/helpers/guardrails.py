...
class OutputValidator:
    ...
    async def validate_output(self, result: RunResult) -> RunResult:
        ...
        if len(output_str) > self.max_length:
            ...
            output_str = (
                output_str[: self.max_length] + " [truncated by output guardrail]"
            )
...
class SafetyEvaluator:
    ...
    async def _evaluate_input_risks(self, input_msg: str) -> dict[str, Any]:
        ...
        return {"score": min(risk_score, 1.0), "factors": risk_factors}
...
class GuardrailsManager:
    ...
    async def process_input(self, message: str) -> str:
        ...
        try:
            validated_message = await self.input_validator.validate_and_sanitize(
                message
            )
    ...
    async def evaluate_interaction(self, input_msg: str, output: str) -> dict[str, Any]:
        ...
        try:
            evaluation = await self.safety_evaluator.evaluate_interaction(
                input_msg, output
            )
            return evaluation
        except Exception as e:
...
def initialize_guardrails(strict_mode: bool = False) -> None:
    ...
    PrintStyle(font_color="green", padding=True).print("Guardrails system initialized")
