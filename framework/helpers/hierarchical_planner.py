...
from framework.helpers.task_models import PlannedTask, TaskPlan
...
class Subtask(BaseModel):
    ...
    tool_name: str | None = Field(
        default=None, description="Recommended tool/instrument for execution"
    )
    dependencies: list[str] = Field(
        default_factory=list, description="List of subtask IDs that must complete first"
    )
    status: SubtaskStatus = Field(default=SubtaskStatus.PENDING)
    result: str | None = Field(
        default=None, description="Result/output of subtask execution"
    )
    error: str | None = Field(
        default=None, description="Error message if subtask failed"
    )
...
class HierarchicalPlan(BaseModel):
    ...
    subtasks: list[Subtask] = Field(
        default_factory=list, description="Ordered list of subtasks"
    )
    status: PlanStatus = Field(default=PlanStatus.PENDING)
    context_id: str | None = Field(
        default=None, description="Agent context for plan execution"
    )
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC))
    ...
    def is_complete(self) -> bool:
        ...
        return all(
            subtask.status == SubtaskStatus.COMPLETED for subtask in self.subtasks
        )
...
class PlanningConfig(BaseModel):
    ...
    auto_planning_enabled: bool = Field(
        default=True, description="Enable automatic planning"
    )
    max_recursion_depth: int = Field(
        default=3, description="Maximum recursion depth for plan decomposition"
    )
    model_name: str = Field(default="o3", description="LLM model to use for planning")
    max_subtasks: int = Field(
        default=10, description="Maximum number of subtasks per plan"
    )
    verification_enabled: bool = Field(
        default=True, description="Enable subtask result verification"
    )
    retry_failed_subtasks: bool = Field(
        default=True, description="Retry failed subtasks with alternative approaches"
    )
...
class HierarchicalPlanner:
    ...
    def create_plan(
        self, objective: str, context_id: str | None = None
    ) -> HierarchicalPlan:
        ...
        plan = HierarchicalPlan(id=plan_id, objective=objective, context_id=context_id)
        ...
        self.printer.print(
            f"Created hierarchical plan '{plan_id}' with {len(subtasks)} subtasks"
        )
        return plan
    ...
    def _decompose_objective(self, objective: str) -> list[Subtask]:
        ...
        elif "create" in objective.lower() and (
            "slide" in objective.lower() or "presentation" in objective.lower()
        ):
    ...
    def _extract_presentation_topic(self, objective: str) -> str:
        ...
        if "machine learning" in objective_lower:
            ...
            key_words = [
                word
                for word in words
                if len(word) > 3 and word not in ["create", "about", "make"]
            ]
            if key_words:
    ...
    def _create_generic_plan(self, objective: str) -> list[Subtask]:
        ...
        if any(
            ...
            for word in ["search", "find", "research", "investigate"]
        ):
            ...
            word in objective_lower
            for word in ["create", "build", "develop", "generate", "make"]
        ):
            # Creation/development task
            if any(
                word in objective_lower
                for word in ["code", "script", "program", "application", "software"]
            ):
            ...
            word in objective_lower
            for word in ["analyze", "examine", "evaluate", "assess"]
        ):
            # Analysis task
            subtasks = [
                Subtask(
            ...
            word in objective_lower for word in ["summarize", "report", "document"]
        ):
    ...
    def get_plan(self, plan_id: str) -> HierarchicalPlan | None:
    ...
    def execute_plan(self, plan_id: str) -> bool:
        ...
        self.printer.print(
            f"Started execution of plan '{plan_id}' with {len(plan.subtasks)} subtasks"
        )
        return True
