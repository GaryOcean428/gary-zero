...
from framework.helpers.task_models import Task, ScheduledTask, AdHocTask, PlannedTask, TaskState
...
class EnhancedTaskScheduler(BaseTaskScheduler):
    ...
    def _get_task_timeout(self, task: Task) -> Optional[float]:
        """Get task timeout."""
        config = get_orchestration_config()
        return config.default_task_timeout_seconds
    
    def _get_task_agent(self, task: Task) -> Optional[str]:
