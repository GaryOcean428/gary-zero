...
class PlanningTool(Tool if _TOOL_AVAILABLE else object):
    ...
    async def create_plan(self, **kwargs) -> Response:
        ...
        try:
            ...
            if auto_execute and plan.status == PlanStatus.IN_PROGRESS:
                ...
                response_message += f"Enhanced features: evaluation loops, automatic retry on failures.\n"
            else:
    ...
    async def configure_planner(self, **kwargs) -> Response:
        ...
        try:
            ...
            for key in ["auto_planning_enabled", "max_recursion_depth", "model_name", 
                       "max_subtasks", "verification_enabled", "retry_failed_subtasks"]:
                if key in kwargs:
            ...
            updated_config = self.enhanced_scheduler.update_configuration(**config_updates)
            
            message = "✅ Enhanced planner configuration updated:\n"
            for key, value in config_updates.items():
                emoji = "✅" if isinstance(value, bool) and value else "❌" if isinstance(value, bool) else "🔧"
