...
from .secret_store import get_secret, store_secret

logger = logging.getLogger(__name__)


def get_api_key(
    service: str, default: str | None = None, fallback_env_var: str | None = None
) -> str | None:
    """
    ...
    if fallback_env_var:
        ...
        if env_key:
            logger.debug(
                f"Retrieved {service} API key from environment variable {fallback_env_var}"
            )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to store {service} API key in secret store: {e}"
                )
            return env_key
...
def get_auth_credentials(service: str) -> dict[str, str] | None:
    """
    ...
    env_user = os.getenv(f"{service.upper()}_USERNAME") or os.getenv(
        f"{service.upper()}_USER"
    )
    env_pass = os.getenv(f"{service.upper()}_PASSWORD") or os.getenv(
        f"{service.upper()}_PASS"
    )
    ...
    if env_user and env_pass:
        ...
        try:
            ...
            logger.warning(
                f"Failed to store {service} credentials in secret store: {e}"
            )
...
def get_webhook_url(service: str, default: str | None = None) -> str | None:
    """
...
def get_certificate(service: str) -> str | None:
    """
...
def migrate_environment_secrets(
    env_mapping: dict[str, str] | None = None, overwrite: bool = False
) -> int:
    """
...
def get_openai_api_key(default: str | None = None) -> str | None:
    """Get OpenAI API key."""
    return get_api_key("openai", default, "OPENAI_API_KEY")


def get_anthropic_api_key(default: str | None = None) -> str | None:
    """Get Anthropic API key."""
    return get_api_key("anthropic", default, "ANTHROPIC_API_KEY")


def get_google_api_key(default: str | None = None) -> str | None:
    """Get Google API key."""
    return get_api_key("google", default, "GOOGLE_API_KEY")


def get_gemini_api_key(default: str | None = None) -> str | None:
    """Get Gemini API key."""
    return get_api_key("gemini", default, "GEMINI_API_KEY")


def get_groq_api_key(default: str | None = None) -> str | None:
    """Get Groq API key."""
    return get_api_key("groq", default, "GROQ_API_KEY")


def get_perplexity_api_key(default: str | None = None) -> str | None:
    """Get Perplexity API key."""
    return get_api_key("perplexity", default, "PERPLEXITY_API_KEY")


def get_huggingface_token(default: str | None = None) -> str | None:
    """Get Hugging Face token."""
    return get_api_key("huggingface", default, "HUGGINGFACE_TOKEN")


def get_e2b_api_key(default: str | None = None) -> str | None:
    """Get E2B API key."""
    return get_api_key("e2b", default, "E2B_API_KEY")


def get_jwt_secret(default: str | None = None) -> str | None:
    """Get JWT secret."""
...
def get_session_secret(default: str | None = None) -> str | None:
    """Get session secret."""
...
def get_kali_credentials() -> dict[str, str] | None:
...
def get_supabase_config() -> dict[str, str] | None:
    ...
    service_role_key = get_secret("supabase_service_role_key") or os.getenv(
    ...
    if url and anon_key:
        ...
        try:
            ...
            if service_role_key and not get_secret("supabase_service_role_key"):
                store_secret(
                    "supabase_service_role_key", service_role_key, overwrite=False
                )
        except Exception as e:
...
def require_api_key(service: str, env_var: str | None = None):
    """
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if not api_key:
                raise ValueError(
                    f"No API key found for {service}. Please add it to the secret store or set {env_var} environment variable."
                )
...
class SecretStoreIntegration:
    ...
    def get_secret(
        ...
        if required and not value:
            raise ValueError(
                f"{self.component_name} requires secret '{name}' but it was not found"
            )
        ...
        if value:
            ...
            self.logger.warning(
                f"No value found for secret '{name}' in {self.component_name}"
            )

        return value

    def get_api_key(self, service: str, required: bool = True) -> str | None:
        """
        ...
        return self.get_secret(
            f"{service}_api_key", env_fallback=env_var, required=required
        )
    ...
    def get_credentials(
        ...
        if username and password:
            ...
            raise ValueError(
                f"{self.component_name} requires credentials for {service}"
            )
