...
class SecretUpdateRequest(BaseModel):
    ...
    expires_in_days: Optional[int] = Field(None, description="New expiration in days")
...
async def create_secret(
    ...
    try:
        ...
        # Store the secret
        success = store.store_secret(request.name, request.value, metadata, overwrite=request.overwrite)
...
async def list_secrets(
    ...
    try:
        ...
        for secret_metadata in secrets_list:
            ...
            from framework.security.secret_store import StoredSecret
            ...
            if secret_metadata.rotation_interval_days:
                rotation_due = secret_metadata.updated_at + timedelta(days=secret_metadata.rotation_interval_days)
                needs_rotation = datetime.utcnow() > rotation_due
            
            response_secrets.append(SecretResponse(
...
async def get_secret(
    ...
    try:
        ...
        if metadata.rotation_interval_days:
            rotation_due = metadata.updated_at + timedelta(days=metadata.rotation_interval_days)
            needs_rotation = datetime.utcnow() > rotation_due
...
async def update_secret(
    ...
    try:
        ...
        secrets_list = store.list_secrets(include_metadata=True)
        current_metadata = next((s for s in secrets_list if s.name == secret_name), None)
        ...
        if request.value is not None:
            ...
            updated_metadata = SecretMetadata(
                ...
                description=request.description if request.description is not None else current_metadata.description,
                expires_at=datetime.utcnow() + timedelta(days=request.expires_in_days) if request.expires_in_days is not None else current_metadata.expires_at,
                rotation_interval_days=request.rotation_interval_days if request.rotation_interval_days is not None else current_metadata.rotation_interval_days,
                tags=request.tags if request.tags is not None else current_metadata.tags,
                owner=current_metadata.owner
            )
            
            # Store updated secret
            success = store.store_secret(secret_name, request.value, updated_metadata, overwrite=True)
            if not success:
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        # Check expiration and rotation
        is_expired = updated_secret.expires_at and datetime.utcnow() > updated_secret.expires_at
        needs_rotation = False
        if updated_secret.rotation_interval_days:
            rotation_due = updated_secret.updated_at + timedelta(days=updated_secret.rotation_interval_days)
            needs_rotation = datetime.utcnow() > rotation_due
