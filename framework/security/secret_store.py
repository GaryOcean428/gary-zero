...
class SecretMetadata(BaseModel):
    ...
    secret_type: SecretType = Field(default=SecretType.OTHER, description="Type of secret")
    access_level: AccessLevel = Field(default=AccessLevel.RESTRICTED, description="Access level")
    ...
    owner: Optional[str] = Field(None, description="Owner of the secret")
    ...
    def validate_name(cls, v):
        ...
        if not all(c.isalnum() or c in '_-./' for c in v):
            raise ValueError("Secret name can only contain alphanumeric characters, underscore, hyphen, and dot")
        return v.strip()
...
class StoredSecret(BaseModel):
    ...
    def needs_rotation(self) -> bool:
        ...
        if not self.metadata.rotation_interval_days:
            return False
        rotation_due = self.metadata.updated_at + timedelta(days=self.metadata.rotation_interval_days)
        return datetime.utcnow() > rotation_due


class SecretStoreConfig(BaseModel):
    """Configuration for the secret store."""
    store_path: Path = Field(default=Path("secrets/store.encrypted"), description="Path to store file")
    encryption_key: Optional[bytes] = Field(None, description="Encryption key (auto-generated if not provided)")
...
class InternalSecretStore:
    ...
    def _initialize_encryption(self):
        ...
        if self.config.encryption_key:
            ...
            if key_env:
                ...
                logger.warning("Generated new encryption key. Store SECRET_STORE_KEY environment variable to persist.")
    ...
    def _log_access(self, operation: str, secret_name: str, success: bool = True, 
                   details: Optional[str] = None, user: Optional[str] = None):
        """Log secret access for audit purposes."""
        if not self.audit_logger:
            return
            
        event_type = AuditEventType.SYSTEM_EVENT if success else AuditEventType.SECURITY_VIOLATION
    ...
    def store_secret(self, name: str, value: Union[str, SecretStr], 
        ...
        with self._lock:
            ...
            # Check store limits
            if len(self._secrets) >= self.config.max_secrets and name not in self._secrets:
                raise SecretStoreError(f"Maximum number of secrets ({self.config.max_secrets}) reached")
            ...
            try:
                ...
                self._log_access(operation, name, True, f"Secret {operation}d successfully")
    ...
    def get_secret(self, name: str, access_level: Optional[AccessLevel] = None,
        ...
        with self._lock:
            ...
            if access_level and secret.metadata.access_level == AccessLevel.ADMIN and access_level != AccessLevel.ADMIN:
                self._log_access("get", name, False, "Insufficient access level", user)
                raise SecretAccessDeniedError(f"Insufficient access level for secret '{name}'")
            
            try:
                # Decrypt the secret value
                decrypted_value = self._fernet.decrypt(secret.encrypted_value).decode()
                
                self._log_access("get", name, True, "Secret retrieved successfully", user)
    ...
    def delete_secret(self, name: str, user: Optional[str] = None) -> bool:
        """
        Delete a secret.
        ...
        with self._lock:
            ...
            try:
                ...
                self._log_access("delete", name, True, "Secret deleted successfully", user)
    ...
    def list_secrets(self, include_metadata: bool = True, 
                    user: Optional[str] = None) -> Union[List[str], List[SecretMetadata]]:
        """
        ...
        """
        with self._lock:
            self._log_access("list", "all", True, f"Listed {len(self._secrets)} secrets", user)
    ...
    def update_metadata(self, name: str, metadata: SecretMetadata,
        ...
        with self._lock:
            if name not in self._secrets:
                self._log_access("update_metadata", name, False, "Secret not found", user)
                raise SecretNotFoundError(f"Secret '{name}' not found")
            ...
            try:
                ...
                self._log_access("update_metadata", name, True, "Metadata updated successfully", user)
                logger.info(f"Metadata for secret '{name}' updated successfully")
                return True
                
            except Exception as e:
                self._log_access("update_metadata", name, False, f"Update failed: {e}", user)
                logger.error(f"Failed to update metadata for secret '{name}': {e}")
                raise SecretStoreError(f"Failed to update metadata: {e}")

    def load_from_environment(self, env_mappings: Optional[Dict[str, str]] = None,
                             prefix: str = "", overwrite: bool = False) -> int:
        """
                ...
                if value:
                    try:
                        ...
                        if 'password' in secret_name.lower():
                            ...
                            secret_type = SecretType.DATABASE_URL if 'database' in secret_name.lower() else SecretType.WEBHOOK_URL
                        ...
                        if self.store_secret(secret_name, value, metadata, overwrite=overwrite):
                            loaded_count += 1
                            logger.info(f"Loaded secret '{secret_name}' from environment variable '{env_var}'")
                            
                    except SecretAlreadyExistsError:
                        if not overwrite:
                            logger.debug(f"Secret '{secret_name}' already exists, skipping")
                    except Exception as e:
...
_secret_store: Optional[InternalSecretStore] = None
_store_lock = threading.Lock()


def get_secret_store(config: Optional[SecretStoreConfig] = None) -> InternalSecretStore:
    """
...
def get_secret(name: str, default: Optional[str] = None, 
               access_level: Optional[AccessLevel] = None) -> Optional[str]:
    """
...
def store_secret(name: str, value: Union[str, SecretStr], 
                metadata: Optional[SecretMetadata] = None,
                overwrite: bool = False) -> bool:
    """
