...
class SlackNotifier(Tool):
    ...
    async def _send_message(self) -> Response:
        ...
        if not message:
            return Response(message="❌ Message content is required", break_loop=False)
    ...
    async def _send_notification(self) -> Response:
        ...
        priority_emoji = {"low": "ℹ️", "normal": "📢", "high": "⚠️", "urgent": "🚨"}.get(
            priority, "📢"
        )
    ...
    async def _list_channels(self) -> Response:
        ...
        return Response(message=channels_text, break_loop=False)
