...
class ModernizedTool:
    ...
    def _initialize_api_keys(self):
        ...
        print(
            f"  OpenAI API Key: {'✓ Found' if self.openai_api_key else '✗ Not found'}"
        )
        print(
            f"  Anthropic API Key: {'✓ Found' if self.anthropic_api_key else '✗ Not found'}"
        )
        print(
            f"  Gemini API Key: {'✓ Found' if self.gemini_api_key else '✗ Not found'}"
        )
    ...
    def get_database_config(self):
        ...
        db_url = self.secrets.get_secret(
            "database_url", env_fallback="DATABASE_URL", required=False
        )
    ...
    def get_auth_config(self):
        ...
        jwt_secret = self.secrets.get_secret(
            "jwt_secret", env_fallback="JWT_SECRET", required=False
        )
        session_secret = self.secrets.get_secret(
            "session_secret", env_fallback="SESSION_SECRET", required=False
        )

        return {"jwt_secret": jwt_secret, "session_secret": session_secret}
...
def demonstrate_decorator_pattern():
    ...
    def openai_chat_completion(prompt: str, api_key: str = None):
        ...
        print(
            f"   Making OpenAI request with key: {api_key[:10]}..."
            if api_key
            else "No API key"
        )
    ...
    def gemini_generate(prompt: str, api_key: str = None):
        ...
        print(
            f"   Making Gemini request with key: {api_key[:10]}..."
            if api_key
            else "No API key"
        )
...
if __name__ == "__main__":
    ...
    print(
        "3. Migrate secrets using: python -m framework.security.secret_cli import-env"
    )
