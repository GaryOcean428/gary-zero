...
class PlanningDemo:
    ...
    def demo_configuration(self):
        ...
        updated_config = self.scheduler.update_configuration(
            max_subtasks=8, verification_enabled=True, retry_failed_subtasks=True
        )
    ...
    def demo_battery_research(self):
        ...
        plan = self.scheduler.create_and_execute_plan(
            objective=objective, context_id="demo_context", auto_execute=False
        )
        ...
        for i, subtask in enumerate(plan.subtasks, 1):
            deps = (
                f" (depends on #{', #'.join([str(j + 1) for j, s in enumerate(plan.subtasks) if s.id in subtask.dependencies])})"
    ...
    def demo_evaluation_system(self):
        ...
        plan = self.scheduler.create_and_execute_plan(
            "Research machine learning trends", auto_execute=False
        )
...
"""
    ...
    def demo_plan_monitoring(self, plan):
        ...
        if plan.subtasks:
            ...
            time.sleep(1)
            first_subtask.mark_completed(
                "Successfully completed with detailed output including relevant information and sources."
            )
            ...
            print(
                f"  📊 Updated Progress: {updated_status['completed_subtasks']}/{updated_status['total_subtasks']}"
            )
    ...
    def demo_failure_handling(self):
        ...
        # Create a plan
        plan = self.scheduler.create_and_execute_plan(
            "Research quantum computing developments", auto_execute=False
        )
        ...
        if plan.subtasks:
            ...
            if evaluation_result.requires_retry:
                ...
                if evaluation_result.alternative_tool:
                    print(
                        f"   💡 Suggested alternative tool: {evaluation_result.alternative_tool}"
                    )
