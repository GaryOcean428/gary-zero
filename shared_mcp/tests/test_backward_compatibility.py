...
@pytest.mark.skipif(
    not FRAMEWORK_IMPORTS_OK,
    reason=f"Framework import failed: {FRAMEWORK_IMPORT_ERROR if not FRAMEWORK_IMPORTS_OK else ''}",
)
class TestBackwardCompatibility:
    ...
    def test_dynamic_proxy_creation(self):
        ...
        with unittest.mock.patch(
            "framework.helpers.mcp_server_compat.settings.get_settings"
        ) as mock_settings:
    ...
    def test_mcp_tool_creation(self):
        ...
        tool = MCPTool(
            agent=mock_agent, name="test.tool", method=None, args={}, message=""
        )
