...
def analyze_405_potential():
    ...
    for route in routes:
        ...
        if 'OPTIONS' not in route['methods']:
            issues.append(f"⚠️  {route['path']} missing OPTIONS method (needed for CORS)")
    ...
    for form in forms:
        ...
        if matching_route:
            if form['method'] not in matching_route['methods']:
                issues.append(f"❌ Form {form['action']} uses {form['method']} but route only allows {matching_route['methods']}")
        else:
...
if __name__ == '__main__':
    ...
    if issues or not handlers_ok:
        print(f"\n🎯 Recommended Actions:")
        if issues:
            print(f"  1. Fix route method mismatches")
            print(f"  2. Add missing POST/OPTIONS methods to routes")
            print(f"  3. Verify form actions match available routes")
        if not handlers_ok:
            print(f"  4. Add comprehensive error handlers")
            print(f"  5. Ensure proper CORS/OPTIONS handling")
