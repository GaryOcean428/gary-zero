...
(function() {
    ...
    function testCSPMeta() {
        ...
        const hasExternalDomains = meta?.content.includes('cdnjs.cloudflare.com') && 
                                  meta?.content.includes('fonts.googleapis.com');
        ...
        return {
            ...
            message: meta ? 
                (hasUnsafeEval ? 
                    (hasExternalDomains ? 'CSP properly configured' : 'CSP missing external domains') 
    ...
    function testAlpineEval() {
        try {
            ...
            return {
                ...
                passed: result === true,
                message: result === true ? 'Function constructor works - Alpine.js should function' : 'Function constructor blocked',
    ...
    function testToastManager() {
        const containers = document.querySelectorAll('#toast-container, #toast-container-enhanced, #toast-container-fallback');
        ...
        return {
            ...
            details: Array.from(containers).map(c => c.id).join(', ')
        };
    }
    ...
    function testAlpineComponents() {
        ...
        return {
            ...
            message: hasAlpine ? 
                (hasCustomComponents ? `Alpine.js v${hasVersion} with custom components loaded` : 'Alpine.js loaded but missing custom components') 
