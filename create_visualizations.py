...
def create_performance_timeline_chart(events: List[LogEvent], output_file: str = "performance_timeline.png"):
    """Create a timeline chart showing performance metrics over time."""
    # Filter performance events with duration data
    perf_events = [
        e for e in events 
        if e.duration_ms is not None and e.event_type in [
    ...
    for event_type in set(event_types):
        type_timestamps = [t for i, t in enumerate(timestamps) if event_types[i] == event_type]
        type_durations = [d for i, d in enumerate(durations) if event_types[i] == event_type]
        
        plt.scatter(type_timestamps, type_durations, 
...
def create_benchmark_comparison_chart(results: List[BenchmarkResult], output_file: str = "benchmark_comparison.png"):
    ...
    for i, config_name in enumerate(config_names):
        ...
        bars = ax.bar([xi + offset for xi in x], scores, width, 
                     label=config_name, color=colors[i % len(colors)], alpha=0.8)
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([task.replace('_', ' ').title() for task in task_names], rotation=45, ha='right')
    ax.legend()
...
def create_event_distribution_chart(events: List[LogEvent], output_file: str = "event_distribution.png"):
...
def create_performance_summary_report(events: List[LogEvent], results: List[BenchmarkResult], output_file: str = "performance_summary.html"):
    ...
    if benchmark_stats and "error" not in benchmark_stats:
        ...
        status_class = "status-success" if success_rate > 80 else "status-warning" if success_rate > 60 else "status-error"
    ...
    if benchmark_stats and "error" not in benchmark_stats:
        ...
        if success_rate < 0.8:
            recommendations.append(f"Success rate is {success_rate:.1%}. Investigate failing test cases to improve reliability.")
        
        avg_score = benchmark_stats.get('score_stats', {}).get('mean', 0)
        if avg_score < 0.7:
            recommendations.append(f"Average score is {avg_score:.2f}. Consider model fine-tuning or prompt optimization.")
    ...
    if duration_stats:
        ...
        if slow_operations:
            recommendations.append(f"Slow operations detected: {', '.join(slow_operations)}. Consider performance optimization.")
...
async def generate_sample_data():
    ...
    print(f"✅ Generated {len(await logger.get_events())} log events and {len(results)} benchmark results")
